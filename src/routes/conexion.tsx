...
export const Route = createFileRoute("/conexion")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Preguntas en doble ciega, retos cooperativos y preguntas secretas.",
      },
      ...
      <div className="glass mb-5 grid grid-cols-3 rounded-full p-1">
        {tabs.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`rounded-full py-2 text-sm font-medium transition ${tab === t ? "bg-rose text-primary-foreground" : "text-muted-foreground"}`}
          >
            {t}
          </button>
        ))}
...
function Daily() {
  ...
  const [chat, setChat] = useState<{ by: string; text: string }[]>([
    { by: "Yuki", text: "Jajaja sabía que dirías eso 🥹" },
  ]);
    ...
    <div className="space-y-4">
      <div className="glass rounded-3xl p-5">
        ...
        <h2 className="mt-2 text-3xl font-semibold leading-tight">
          ¿Qué pequeño gesto mío te hizo sentir más querida esta semana?
        </h2>
      </div>
        ...
        {sent ? (
          ...
          <div className="space-y-2">
            <textarea
              ...
              className="w-full resize-none rounded-2xl bg-muted p-3 text-sm outline-none focus:ring-1 focus:ring-ring"
            />
            <button
              disabled={!answer.trim()}
              onClick={() => setSent(true)}
              className="w-full rounded-full bg-rose py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-40"
            >
              Enviar y revelar
            </button>
          </div>
      ...
      <div className="glass relative overflow-hidden rounded-3xl p-4">
        ...
        <p className={`text-sm transition-all duration-700 ${sent ? "" : "select-none blur-md"}`}>
          Cuando me mandaste el audio cantando a las 3 de la mañana solo para que me durmiera. Me
          hizo llorar de lo bonito.
        </p>
        {!sent && (
...
function Challenges() {
  const [items, setItems] = useState([
    {
      icon: ChefHat,
      title: "Cocinar lo mismo",
      desc: "Preparad la misma receta y cenad en videollamada.",
      me: true,
      them: false,
    },
    {
      icon: Camera,
      title: "Foto espontánea",
      desc: "Mandad una foto de lo que veis ahora mismo, sin filtros.",
      me: true,
      them: true,
    },
    {
      icon: Music,
      title: "Playlist cruzada",
      desc: "Añadid 5 canciones que os recuerden al otro.",
      me: false,
      them: true,
    },
    {
      icon: Moon,
      title: "Misma luna",
      desc: "Fotografiad la luna desde vuestra ciudad esta noche.",
      me: false,
      them: false,
    },
  ]);
        ...
        return (
          <div key={c.title} className={`glass rounded-3xl p-4 ${done ? "shadow-glow" : ""}`}>
            <div className="flex gap-3">
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-muted">
            ...
            <div className="mt-3 flex items-center justify-between">
              <div className="flex gap-2 text-[11px]">
                <span
                  className={`rounded-full px-2 py-0.5 ${c.me ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                >
                  Tú {c.me ? "✓" : "…"}
                </span>
                <span
                  className={`rounded-full px-2 py-0.5 ${c.them ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                >
                  Yuki {c.them ? "✓" : "…"}
                </span>
              </div>
              {done ? (
                ...
                <button
                  onClick={() => setItems(items.map((x, j) => (j === i ? { ...x, me: !x.me } : x)))}
...
function Secret() {
  const [qs, setQs] = useState([
    {
      q: "¿Cuál fue el primer momento en que supiste que me querías?",
      from: "Yuki",
      answered: false,
    },
    ...
    <div className="space-y-3">
      <div className="glass rounded-3xl p-4">
        <p className="mb-2 flex items-center gap-2 text-sm font-semibold">
          ...
          Crea una pregunta secreta
        </p>
        <div className="flex gap-2">
          <input
            value={text}
            onChange={(e) => setText(e.target.value)}
            placeholder="Solo Yuki podrá verla…"
            className="min-w-0 flex-1 rounded-full bg-muted px-4 py-2 text-sm outline-none"
          />
          <button
            onClick={() => {
              if (text.trim()) {
                setQs([{ q: text, from: "Tú", answered: false }, ...qs]);
                setText("");
              }
            }}
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-rose text-primary-foreground"
