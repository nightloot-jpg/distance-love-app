...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Vuestras horas, el contador del reencuentro y un latido en directo.",
      },
        ...
        {people.map((p) => (
          <div key={p.name} className="glass rounded-3xl p-4">
            <div className="flex items-center gap-2">
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-rose font-display text-lg font-bold text-primary-foreground">
                {p.name[0]}
              </div>
            ...
            <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                <p.Weather className="h-3.5 w-3.5 text-primary" />
                {p.temp}
              </span>
              <span className="flex items-center gap-1">
                <p.Bat className="h-3.5 w-3.5" />
                {p.battery}%
              </span>
            </div>
          </div>
    ...
    <section className="my-8 flex flex-col items-center">
      <div className="relative grid h-44 w-44 place-items-center">
        {rings.map((id) => (
          <span
            key={id}
            className="absolute h-24 w-24 rounded-full border-2 border-primary animate-ripple"
          />
        ))}
        <button
          onPointerDown={start}
      ...
      <p className="text-sm text-muted-foreground">
        {holding ? "Yuki siente tu latido…" : "Mantén pulsado para enviar tu latido"}
      </p>
    </section>
  );
}
    ...
    <section className="glass rounded-3xl p-4">
      ...
      <div className="mb-3 flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-full bg-muted px-4 py-2 text-sm outline-none focus:ring-1 focus:ring-ring"
        />
        <button
          onClick={add}
        ...
        {notes.map((n) => (
          <li key={n.id} className="flex items-center gap-3 rounded-2xl bg-muted/60 px-3 py-2.5">
            <button
              ...
              className={`grid h-5 w-5 shrink-0 place-items-center rounded-full border border-primary ${n.done ? "bg-primary" : ""}`}
            >
              {n.done && <Check className="h-3 w-3 text-primary-foreground" />}
            </button>
            <div className="min-w-0 flex-1">
              <p className={`text-sm ${n.done ? "text-muted-foreground line-through" : ""}`}>
                {n.text}
              </p>
